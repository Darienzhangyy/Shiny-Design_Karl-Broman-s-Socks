    ...
    priors = reactive(
      {
        ...
        { 
          #use parallelization to reduce the execution time
          d_n_sock = unlist(mclapply(1:10, 
                                     function(x) rpois(input$n_sim/10, input$total_lambda), 
                                     mc.cores = 10))
          #d_n_sock = rpois(input$n_sims, input$total_lambda)
        } else {
          d_n_sock = unlist(mclapply(1:10, 
                                     function(x) rnbinom(input$n_sims/10,size = input$total_r, prob = input$total_p),
                                     mc.cores = 10))
