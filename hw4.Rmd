...
To implement the algorithm of ABC, we first create a prior function that allows user to choose priors as discussed above. We then calculate the priors for `n_pairs` (number of paired socks) and `n_odd` (number of odd socks) given user defined hyperparameters to get prior distributions of `n_socks`(number of total socks) and `prop_pairs`(proportion of paired socks). We then simulate the data to create a column of simulated number of pairs, which can be used later to be compared with user-input `n_paired` to get the rows to get the posterior distribution. The posterior function implements it by picking out the rows from the dataframe result from prior() where number of paired socks simulated equals number of pairs a user inputs,`n_paired`. We then plot eight histograms of mentioned four variables of interest above and provide summary statistics for those four variables. Finally, the prediction of number of total socks and paired socks interacts with observed number of total and paired socks. Specifically, when total 11 singletons and 0 paired socks is observed , Our web app give a prediction of 44 total socks (19 pairs+6 odds) mentioned in Rasmus’ bog. 
